...
class Admin_categories extends Admin_Controller
{
    ...
    {
        $extra['title'] = 'lang:dossiers:categories';
        
        $extra['buttons'] = array(
            array(
                'label' => lang('global:edit'),
                'url' => 'admin/dossiers/categories/edit/-entry_id-'
            ),
            array(
                'label' => lang('global:delete'),
                'url' => 'admin/dossiers/categories/delete/-entry_id-',
                'confirm' => true
            )
        );
        
        //entries_table($stream_slug, $namespace_slug, $pagination = null, $pagination_uri = null, $view_override = false, $extra = array())
        $this->streams->cp->entries_table('categories', 'dossiers', 20, 'admin/dossiers/categories/index', true, $extra);
    }
    ...
    public function create()
    {
        ...
        $extra = array(
            'return' => 'admin/dossiers/categories/index',
            'success_message' => lang('dossiers:submit_success'),
            'failure_message' => lang('dossiers:submit_failure'),
            'title' => lang('dossiers:categories:new')
        );

        $skips = array('cat_dom_id');
        $this->streams->cp->entry_form('categories', 'dossiers', 'new', null, true, $extra, $skips);
    }
    ...
    public function edit($id = 0)
    {
        ...
        $skips = array('cat_dom_id');
        $this->streams->cp->entry_form('categories', 'dossiers', 'edit', $id, true, $extra, $skips);
    }
    ...
    {
        $this->streams->entries->delete_entry($id, 'categories', 'dossiers');
        $this->session->set_flashdata('error', lang('dossiers:deleted'));
        redirect('admin/dossiers/categories/index');
    }
