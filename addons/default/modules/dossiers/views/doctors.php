...
<div id="dossiers-list">
	...
	<div class="h3 text-center" >
        ...
        {{ if dossiers.entries }}
        <div class="listing"> 
		<!-- start dossiers entries --> 
                <?php 
                foreach ($dossiers['entries']as $docObj => $doc) 
                {       
                        // Start foreach loop
                        $docCount +=1; //counter   
//                        $dossiers;
//                        $doc;
		...
		<section class="dossiers well" data-rid="<?php echo $doc['id'] ?>" data-town="<?= $doc['town'] ?>" data-address="<?= $doc['address'] ?>" data-area_name="<?= $doc['area_name'] ?>" data-name="<?php echo $doc['name'] ?>">
                    <div class="row-fluid">
                            <div class="col-sm-4 col-xs-4"> 
                                ...
                                <?php 
                                    ...
                                    elseif( $doc['dossiers_cat']['speciality'] ) { //default image for speciality
                                        switch ( $doc['dossiers_cat']['speciality'] ) 
                                        {
                            ...
                            <div class="col-sm-4"> 
                                <!--<h4><a href="dossiers/view/<?= $doc['id'] ?>"><?= $doc['name'] ?></a>-->
                                <h4>
                                    <!--<a href="dossiers/view/<?= $doc['id'] ?>"><?= $doc['name'] ?></a>-->
                                    <?= $doc['name'] ?>
                                <?php  
                                    if($doc['dossiers_cat']['speciality']) echo " &bullet; " . $doc['dossiers_cat']['speciality'];
                                    if($doc['dossiers_cat']['speciality'] and $doc['groupe']['subset']) echo " &bullet;&nbsp;";
    ...
    <!--                            <p>
                        ...
                        {{ if user:has_cp_permissions }} 
                                <a target="_blank" class="adminlink" href="<?= site_url() ?>admin/dossiers/edit/<?= $doc['id'] ?>">Modifier</a>
                        {{endif}}
		</section>
